...
// Renders the ufoData to the tbody
function renderTable() {
    tbody.innerHTML = "";
    for (let i = 0; i < ufoData.length; i++) {
        // Get current ufo info
        let info = ufoData[i];
        let fields = Object.keys(info);
        // Create a new row in the tbody
        let row = tbody.insertRow(i);
        for (let j = 0; j < fields.length; j++) {
            // Create new cell and set its inner text to be the current value
            let field = fields[j];
...
function handleSearchClick() {
    // Format user search input by eliminating whitespace and making it lowercase
